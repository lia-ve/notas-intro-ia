...
Las **heurísticas** son la fuerza impulsora que permite estimar la distancia hacia los estados objetivo. Son funciones que toman un estado como entrada y devuelven una estimación de la distancia al objetivo. El cálculo realizado por estas funciones es específico para el problema de búsqueda que se está resolviendo. Por razones que veremos en la **búsqueda A***, generalmente queremos que las funciones heurísticas sean una cota inferior de la distancia restante al objetivo. Por lo tanto, las heurísticas suelen ser soluciones a problemas **relajados** (donde algunas restricciones del problema original se han eliminado).
